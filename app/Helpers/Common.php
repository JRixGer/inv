...
class MP {
  static $auth_token  = "8HIbbV4paQGEbAMrUMaZp0uoK1MzzbFEf2kYs-SxWuE-Q6n_0mArwg";
...
function maroPost_fn($receipt)
{
  ...
  foreach ($maro as $key => $m) 
  {
    ...
    /*
    $mod_item = $m->itemNo;
    $mod_item2 = preg_replace('/\d/', '', $mod_item );
  
    
    $camp_id = DB::table('skus')
    ->select('campaign_id')
    ->where('prodCode', 'LIKE', '%'.$mod_item2.'%')
    ->where('campaign_id', '<>', '' )
    ->first();
    //dd($camp_id);
    //print_r($camp_id);
...
/*
    if ($transactionType == 'ABANDONED_ORDER')
    {
      ...
      $auth_code = "Blft0gQ-XXccABwdFdDdABKf7wqFE3HXfQgwlLtE3gIGgo5YdARhMg";
      $curl = curl_init();
    
      curl_setopt_array($curl, array(
        CURLOPT_URL => "http://api.maropost.com/accounts/1044/journeys/120/trigger/8750061843439.json?email=".$email."&auth_token=".$auth_code."",
